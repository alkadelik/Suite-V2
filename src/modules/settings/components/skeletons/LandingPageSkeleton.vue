<template>
  <section>
    <div class="mb-4 flex items-center gap-6 border-b border-gray-200 pb-4">
      <div class="flex-1">
        <div class="mb-2 h-6 w-28 animate-pulse rounded bg-gray-200"></div>
        <div class="h-4 w-48 animate-pulse rounded bg-gray-200"></div>
      </div>
      <div class="h-9 w-9 animate-pulse rounded-lg bg-gray-200"></div>
      <div class="hidden h-10 w-28 animate-pulse rounded-lg bg-gray-200 md:block"></div>
    </div>
      ...
      <aside class="hidden w-2/5 flex-shrink-0 md:block">
        <div class="space-y-2">
          <div
            v-for="i in 8"
            :key="i"
            class="h-14 w-full animate-pulse rounded-lg bg-gray-200"
          ></div>
        </div>
      </aside>

      <div class="flex flex-col gap-3 md:hidden">
        <div v-for="i in 8" :key="i" class="h-14 w-full animate-pulse rounded-lg bg-gray-200"></div>
      </div>
