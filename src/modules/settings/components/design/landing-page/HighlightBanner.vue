...
// Get highlight banner section from landing page data
const highlightBannerSection = computed(() => {
  const sections = landingPageData.value || []
  for (const section of sections) {
    const item = section.items?.find((i) => i.id === "highlight-banner")
...
// Initial form values
const initialValues = reactive<HighlightBannerFormData>({
  headline: "",
  body_text: "",
  cta_button_text: "",
  cta_button_link: "",
...
// Form submit handler
const handleFormSubmit = (values: HighlightBannerFormData) => {
  const formData = new FormData()
  formData.append("id", "highlight-banner")
  ...
  updateSection(
    {
      id: highlightBannerSection.value?.sectionUid || "",
