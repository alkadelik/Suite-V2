...
// Get CTA block 1 section from landing page data
const ctaBlock1Section = computed(() => {
  const sections = landingPageData.value || []
  for (const section of sections) {
    const item = section.items?.find((i) => i.id === "cta-block-1")
...
const handleFormSubmit = (values: CTABlockFormData) => {
  if (!ctaBlock1Section.value?.sectionUid) {
    toast.error("Section not found")
    return
  }

  const formData = new FormData()
  formData.append("id", "cta-block-1")
  ...
  updateSection(
    {
      id: ctaBlock1Section.value.sectionUid,
